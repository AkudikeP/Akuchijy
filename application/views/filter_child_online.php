...
<div class="product-preview-wrapper col-md-4 col-sm-4 col-xs-6" id="filterproducts_child">
 ...
 <div class="sizes-example sizes-example1 image" >
	 <?php $url =  base_url().'Welcome/boutique_product/'.$fab->main_category.'/'.$fab->id; ?>
   ...
   if(!empty($fab->url))
  {
    ...
    $title_url = preg_replace('/[^A-Za-z0-9 -]/u','', strtolower(strip_tags(str_replace(" ", "-",$fab->brand))));
  }
  
	 if(!empty($mcatname)){
	 $url =  str_replace("--","-",base_url().'online-boutique/'.$catname->mcat_name.'/'.str_replace(" ", "-",$mcatname->name).'/'.$title_url.'/'.$fab->id);
	 }else{
	 $url =  str_replace("--","-",base_url().'online-boutique/'.$catname->mcat_name.'/'.$title_url.'/'.$fab->id);
	 } ?>
                <a href="<?php echo $url;?>">
                   <?php if($fab->thumb=='' || $fab->thumb==' '){ ?>
               <img src="<?php echo base_url();?>assets/images/accessrics/cover.jpg" alt=""/></a>
                <?php } else { ?>
                 <img src="<?php echo base_url();?>assets/images/online_boutique/<?php echo $fab->thumb;?>" alt=""/></a>
                <?php } ?>

                <?php if($this->session->userdata("userid")==''){ ?>

                <a href="<?php echo base_url() ?>Welcome/login?url=<?php echo $url; ?>"><div class="overlay_img" id="<?php echo $fab->id; ?>_<?php echo $fab->brand; ?>"><img src="<?php echo base_url(); ?>assets/images/heart_empty.png"></div></a>
                    ...
                    <?php
											...
											<div class="product-preview__info">
												...
												<div class="product-preview__info__title">
													...
													<h2><a href="#"><?php echo substr($fab->brand, 0,20);
                   ...
                   $(".overlay_img").click(function(){
                        ...
                        data: {'pid':id[0],'pname':id[1],'type':'online'},
                         success: function(response){
                          console.log("#" + id[0]);
                          $("#k" + id[0]).css('border','0px solid red');
