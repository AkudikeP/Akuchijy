...
<!-- vd_panel-menu --> 
    ...
    <div class="pd-20">
      <h5 class="mgbt-xs-20 mgtp-20"><span class="menu-icon append-icon"><i class="icon-graph"></i></span> <strong>Order Per Day</strong> (<?php echo date('F')?>) </h5>
    ...
    <a class="panel-body" href="#">
            <div class="clearfix">
                ...
                <span class="menu-value">
                    <?php echo $this->db->get("orders")->num_rows();?>
                </span>  
            </div>   
...
<!-- vd_panel-menu --> 
    ...
    <a class="panel-body"  href="#">                                  
        <div class="clearfix">
            ...
            <span class="menu-value"> 
                 <?php echo $this->db->get_where("orders",array("ostatus"=>"Completed"))->num_rows();?>
            </span>  
        </div>   
        <div class="menu-text clearfix">
            Completed&nbspOrders
        </div>
     </a>                                                                  
</div>                   </div>
                  <!--col-xs-6 --> 
					...
					if (item) {
						if (previousPoint != item.dataIndex) {
							...
							showTooltip(item.pageX, item.pageY,
								"<p class='vd_bg-green'><strong class='mgr-10 mgl-10'>" + Math.round(x)  + " <?php echo date('M Y')?> </strong></p>" +
...
for($i=1;$i<=$day;$i++)
  {
    $last_month_first = $i.'-'.$month.'-'.$year;
    $this->db->where('odate ="'.$last_month_first.'"');

 $num_of_orders = $this->db->get("orders")->num_rows();

    echo "[$i,$num_of_orders]";
    if($i!=$day)
    {
      echo ",";
    }
...
*/
/* PIE CHART */
var tailor = <?php echo $tailor_cost_per;?>;
var order = 100-<?php echo $tailor_cost_per;?>;
		var pie_placeholder = $("#pie-chart");

		var pie_data = [];
		
		pie_data[0] = {
			label: "Stitching Earning",
			data: tailor
		}
		pie_data[1] = {
		...
		$.plot(pie_placeholder, pie_data, {
			...
			colors: ["#CCCCCC", "#52D793", "#56A2CF", "#52D793", "#FC8660", "#CCCCCC"]
		});
