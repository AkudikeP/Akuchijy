...
//print_r($_GET);
        ...
        if (isset($_GET['page_no'])) { //echo "bhen";
            /*$data['id'] = $_POST['id'];
            ...
            foreach ($data['items'] as $fab) {
                if (!empty($fab)) {
                    ...
                    foreach ($bundle as $value4) {
                        ...
                        if (!empty($count_ss)) {$s_qty += $value4->qty;}}
                    $qty = $fab->quantity;
        ...
        if (!empty($data['items'])) {
            ...
            foreach ($data['items'] as $fab) {
                if (!empty($fab)) {
                    ...
                    $html .= '</td><td style="padding:1%">';
                    if ($fab->main_category == 1) {
                        $html .= "Women";
                    }
                    if ($fab->main_category == 2) {
                        $html .= "Men";
                    }
                    if ($fab->main_category == 3) {
                        $html .= "Kids";
                    }
        ...
        if (!empty($data['items'])) {
            ...
            foreach ($data['items'] as $fab) {
                if (!empty($fab)) {
                    ...
                    $html .= $fab->uni_category;
                    $html .= '</td><td>' . $fab->quantity . '</td><td>';
                ...
                foreach ($item as $item) {
                    ...
                    if ($item2_num > 1) {
                        ...
                        foreach ($item2_result as $value) {
                            ...
                            //$value->pid;
                            if ($type[0] == 'accessories') {$a[] = "PAMD-" . $value->pid;} else if ($type[0] == 'fabrics') {$a[] = "PFMD-" . $value->pid;} else if ($type[0] == 'uniform') {$a[] = "PUMD-" . $value->pid;} else if ($type[0] == 'online_boutique') {$a[] = "POMD-" . $value->pid;}
...
$order_date = $this->db->get_where("orders", array("oid" => $item->oid))->row();
                            ...
                            if (!empty($a)) {echo implode(' , ', $a);} else { $type = explode('/', $value->pimg);
                                //$value->pid;
                                if ($type[0] == 'accessories') {echo "PAMD-" . $value->pid;} else if ($type[0] == 'fabrics') {echo "PFMD-" . $value->pid;} else if ($type[0] == 'uniform') {echo "PUMD-" . $value->pid;} else if ($type[0] == 'online_boutique') {echo "POMD-" . $value->pid;}}?></td>
...
$("#"+response_id).html(response);
    ...
    window.location = 'product/Make_addons';
            ...
            //echo $id;die;
            $this->db->where('id', $id);
            if ($this->db->update("addons", $data)) {$url = base_url() . "product/Make_addons";
                echo "<script>
    window.location = '$url';
</script>";
            }
...
<center><a href="#"><img src="' . base_url() . '/assets/sociallinks/facebook_circle_black-24.png"></a><a href="#"><img src="' . base_url() . '/assets/sociallinks/twitter_circle_black-24.png"></a><a href="#"><img src="' . base_url() . '/assets/sociallinks/google_circle_black-24.png"></a></center></p>
        ...
        if ($_POST['shipping_status'] == 'Pickup From Vendor') {
            ...
            if ($this->db->update('orders', $data)) {echo $this->db->last_query();
                foreach ($pinfo as $value) {
            ...
            $data = array("shipping_status" => $_POST['shipping_status'],
                ...
                'notify_status' => 'no');
            $this->db->where("oid", $id);
            if ($this->db->update('orders', $data)) {foreach ($pinfo as $value) {
        ...
        } elseif ($_POST['shipping_status'] == 'Pickup From Vendor') { //echo "in user"; exit;
            if (!empty($uinfo->mobile)) {
                $authKey = "136895AdMGPnqo6n5875df12";
                $mobileNumber = $uinfo->mobile;
                $senderId = "ANSVEL";
        ...
        } elseif ($_POST['shipping_status'] == 'Pickup From User') { //echo "in user"; exit;
            if (!empty($uinfo->mobile)) {
                $authKey = "136895AdMGPnqo6n5875df12";
                $mobileNumber = $uinfo->mobile;
                $senderId = "ANSVEL";
...
//if($tailor_assign<=0)
    ...
    public function get_response_stitching_orders()
    {if ($this->db->get_where('tailor_assign', array("stid" => $_POST['oid'], "sstatus" => 'accepted'))->num_rows() > 0) {echo '1';}}
...
<?php $k = 5;
                            $star = "";for ($j = 1; $j <= $value->overall_rating; $j++) {
            ...
            if ($this->input->post("title")) {
                ...
                if (!empty($_FILES["fabricc"]["name"])) {
                    ...
                    if (!$this->upload->do_upload('fabricc')) {
                        echo $this->upload->display_errors();
                    } else { $this->load->library('image_lib');
                        $image_data = $this->upload->data();
                        $cover = $image_data['file_name'];
                        $configer = array(
                            'image_library' => 'gd2',
        ...
        if ($edit == true) {
            ...
            if ($this->input->post("main_category")) {
                ...
                $data = array("main_category" => $this->input->post("main_category"),
                    ...
                    "category_for_filter" => $this->input->post("fabric_main_cat"),
                    "padddate" => date("Y-m-d"));
                if ($this->db->insert('accessories', $data)) {$this->session->set_flashdata('message', 'Product Saved successfully.');
                    $insert_id = $this->db->insert_id();
                    $search_data = array("product_id" => $insert_id,
                        "filter_subcategory_fcid" => implode(",", $this->input->post("accessories_search")));
                    $this->db->insert('accessories_search', $search_data);
