    ...
    //echo print_r($this->session->userdata);
      $data=array(
            ...
            if($this->db->insert("review",$data))
            {
              ...
              echo "<div >
                 <span class='label " ; if($rating->rating==5){echo 'label-success ';}
...
$keyword = $this->session->userdata('city_session');
    $this->db->where("city LIKE '%$keyword%'");
     ...
     $this->db->limit(5);
  $fab=$this->db->get_where("fabric",array("status"=>'approve',"status_enable"=>'enable'))->result_array();
  if(empty($fab))
  {
    $i++;
  }
foreach ($fab as $value) {
  echo "<option value='".$value['title']." from Fabric'>";
  }
  $keyword = $this->session->userdata('city_session');
    $this->db->where("city LIKE '%$keyword%'");
...
$fab=$this->db->get_where("accessories",array("status"=>'approve',"status_enable"=>'enable'))->result_array();
 if(empty($fab))
  {
    $i++;
  }
  foreach ($fab as $value) {
   echo "<option value='".$value['brand']." from Accessories'>";
  }
  $keyword = $this->session->userdata('city_session');
    $this->db->where("city LIKE '%$keyword%'");
  ...
  $this->db->limit(5);
  $this->db->order_by('id','desc');
    $fab=$this->db->get_where("uniform",array("status"=>'approve',"status_enable"=>'enable'))->result_array();
    if(empty($fab))
  {
    $i++;
  }
  foreach ($fab as $value) {
    echo "<option value='".$value['school_name']." from Uniform'>";
    }
  $keyword = $this->session->userdata('city_session');
    $this->db->where("city LIKE '%$keyword%'");
    $this->db->where("brand LIKE '$val%'");
    $this->db->limit(5);
    $fab=$this->db->get_where("online_boutique",array("status"=>'approve',"status_enable"=>'enable'))->result_array();
    if(empty($fab))
  {
    $i++;
  }
  foreach ($fab as $value) {
   echo "<option value='".$value['brand']." from Online Boutique'>";
  }
  ...
  public function add_city_session()
    ...
    //error_reporting(1);
      //print_r($_POST);
        ...
        //print_r($_SESSION);
//exit;
      ?><script>window.location.href='http://mobiledarzi.com/';</script><?php
    }
...
{   //$this->db->limit(20);
  ...
  public function emptycart(){
	  $this->session->unset_userdata('dis');$this->cart->destroy();
  ...
  public function book_predesign(){
    ...
    <center><div class="products-widget__item"><div class="products-widget__item__image sizes-example1" style="width:200px">
  ...
  $title = explode('_', $allKeys[3]);
	$data = array(
               'rowid'   => $bundle_no,
			         'id'      => $bundle_no,
               'qty'     => 1,
			         'img'     => 'styles/resized/small/'.$image_to_show[0], //'fabrics/'.$fp->thumb,
               'order_type'=>$stitch,
               'price'   => round($total),
			         'custom'   => $c,
               'name'    => $title[0].' Bundle '.$fabric_name,
               'vendor_id'  => $vendor_id,
			         'measures'=>array($mdata),
  ...
  $data=array("name"=>$userNode['name'],
       ...
       if($name = $this->db->get_where("users",$data2)->row())
       {
         $this->session->set_userdata("userid",$name->uid);
          ...
          if(isset($_SESSION['wish_url']))
          { $url = $_SESSION['wish_url'];
            redirect("$url");
          }
          else{
...
//print_r($_POST);die;
	if($this->session->userdata("dis")){
		 ...
		 $ctot= $this->cart->total(); $dis=0;}//exit;
...
<input type="hidden" name="<?php echo $coll1->attr_name;?>1" id="sttt10224" value="" />
    ...
    margin-bottom: 10px;margin-top: 10px; color:#fff; padding:12px;">Total Price</h4></center>
                     ...
                     <?php
                      ...
                      /*if(!is_null($_SESSION['total_price_final']))
                     {
                     	echo $_SESSION['total_price_final'];
                     	$_SESSION['total_price_final'] = 0;
                     }*/
                      ?></h2>
                     </div></div><?php
  }
