 ...
 $this->db->select('*');
  ...
  $user=$this->db->get()->result();
        foreach ($user as $user) {
          ...
          $data1->Date = $order_date->odate;
           $type = explode('/',$user->pimg); if($type[0]=='accessories'){$data1->Product_ID =  "PAMD-".$user->pid; }
                          else if($type[0]=='fabrics'){$data1->Product_ID =  "PFMD-".$user->pid; }
                          else if($type[0]=='uniform'){$data1->Product_ID =  "PUMD-".$user->pid; }
                          else if($type[0]=='online_boutique'){$data1->Product_ID =  "POMD-".$user->pid; } 
...
if(!$this->session->userdata("vendor_loggedin"))
  ...
  $user=$all->result();
        foreach ($user as $user) {
          ...
          if($user->category==1) $data1->Category = "Women"; if($user->category==2) $data1->Category = "Men" ;if($user->category==3) $data1->Category = "Kids" ;
...
if(!$this->session->userdata("vendor_loggedin"))
  ...
  $user=$all->result();
        foreach ($user as $user) {
          ...
          if($user->main_category==1) $data1->Category = "Women"; if($user->main_category==2) $data1->Category = "Men" ;if($user->main_category==3) $data1->Category = "Kids" ;
          $data1->Price = $user->price;
                    $data1->Quantity = $user->quantity;
        ...
        foreach ($user as $fab) {
          ...
          //
                          ...
                          $type = explode('/',$order_item->pimg);
                           if($type[0]=='accessories'){$data1->Product_ID = "PAMD-".$order_item->pid; }
                              else if($type[0]=='fabrics'){$data1->Product_ID = "PFMD-".$order_item->pid; }
                              else if($type[0]=='uniform'){$data1->Product_ID = "PUMD-".$order_item->pid; }
                              else if($type[0]=='online_boutique'){$data1->Product_ID = "POMD-".$order_item->pid; }
...
<td>$item->pname (";
                             ...
                             if($type[0]=='accessories'){$html.= "PAMD-".$item->pid; }
                              else if($type[0]=='fabrics'){$html.= "PFMD-".$item->pid; }
                              else if($type[0]=='uniform'){$html.= "PUMD-".$item->pid; }
                              else if($type[0]=='online_boutique'){$html.= "POMD-".$item->pid; }  $html.=") </td>
<td>
  ...
  else
  {
	...
	if($this->form_validation->run() == FALSE)
      ...
      $this->session->set_flashdata('msg', 'Invalid Email or Password.');
		...
		{
			$chk=$this->db->get_where("vendor",array("approve_status"=>"yes","email"=>$this->input->post("email"),"password"=>md5($this->input->post("password"))))->row();
    ...
    public function add_fabric($edit=false)
  {
	  ...
	  else
	  {
		  if($this->input->post("title")){
			...
			$config['max_size']	= '1000000';
			 $this->upload->initialize($config);
			$cover="cover.jpg";$front="front.jpg";$back="back.jpg";$sizing_guide="size.jpg";
			if(!empty($_FILES["fabricc"]["name"])){
